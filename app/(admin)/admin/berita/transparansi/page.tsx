...
import { DialogNews } from "@/components/admin/news/NewsFormDialog";
import { useState, useEffect, useCallback } from "react";
import { NewsCard } from '@/components/admin/news/NewsCard';
import { LoadingView } from '@/components/admin/news/LoadingView';
import { ErrorView } from '@/components/admin/news/ErrorView';
...
export default function NewsPage() {
  ...
  const fetchNews = useCallback(async () => {
    try {
      ...
      setError("Gagal memuat data berita transparansi");
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="w-full mx-auto flex justify-between items-center">
            ...
            <form onSubmit={handleSearch} className="flex gap-2">
              <div className="relative flex-1">
                <Input
                  type="text"
                  placeholder="Cari berita transparansi..."
                  value={searchInput}
            ...
            {newsData.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-gray-500">
                  {currentSearch
                    ? `Tidak ditemukan berita transparansi dengan kata kunci "${currentSearch}"`
                    : 'Tidak ada berita transparansi tersedia'}
                </p>
                {currentSearch && (
