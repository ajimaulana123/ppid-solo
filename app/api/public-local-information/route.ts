...
export async function GET(request: Request) {
  try {
    ...
    const countSql = search
      ? sql`SELECT COUNT(*) FROM "PublicLocalInformation" 
    ...
    const dataQuery = db
      .select()
      .from(publicLocalInformation);

    if (search) {
      dataQuery.where(
        like(publicLocalInformation.title, `%${search}%`) // Search by title only
      );
    }

    const documentData = await dataQuery
      .orderBy(publicLocalInformation.createdAt)
      .limit(limit)
      .offset((page - 1) * limit);
