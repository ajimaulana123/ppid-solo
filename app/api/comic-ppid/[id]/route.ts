...
export async function PUT(
  ...
  try {
    ...
    const updatedNews = await db.update(activiesComicPpid)
      .set(updateData)
      .where(eq(activiesComicPpid.id, id))
      .returning();
...
export async function DELETE(
  ...
  try {
    ...
    const [newsItem] = await db.select()
      .from(activiesComicPpid)
      .where(eq(activiesComicPpid.id, newsId)) // Now comparing number with number
      .limit(1);
    ...
    await db.delete(activiesComicPpid)
      .where(eq(activiesComicPpid.id, newsId));
