...
export async function GET(
  ...
  try {
    const singleNews = await db
      ...
      .where(eq(activiesVideoPpid.id, Number(params.id)))
      .limit(1);
...
export async function PUT(
  ...
  try {
    ...
    const updatedNews = await db.update(activiesVideoPpid)
      .set(updateData)
      .where(eq(activiesVideoPpid.id, id))
      .returning();
...
export async function DELETE(
  ...
  try {
    ...
    const [newsItem] = await db.select()
      .from(activiesVideoPpid)
      .where(eq(activiesVideoPpid.id, newsId)) // Now comparing number with number
      .limit(1);
    ...
    await db.delete(activiesVideoPpid)
      .where(eq(activiesVideoPpid.id, newsId));
