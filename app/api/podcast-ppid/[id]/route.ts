...
export async function PUT(
  ...
  try {
    ...
    const updatedNews = await db.update(activiesPodcastPpid)
      .set(updateData)
      .where(eq(activiesPodcastPpid.id, id))
      .returning();
...
export async function DELETE(
  ...
  try {
    ...
    const [newsItem] = await db.select()
      .from(activiesPodcastPpid)
      .where(eq(activiesPodcastPpid.id, newsId)) // Now comparing number with number
      .limit(1);
    ...
    await db.delete(activiesPodcastPpid)
      .where(eq(activiesPodcastPpid.id, newsId));
