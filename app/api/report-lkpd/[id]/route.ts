...
import { reportLkpd } from '@/lib/schemaDb';
...
export async function GET(
  ...
  try {
    const singleNews = await db
      .select()
      .from(reportLkpd)
      .where(eq(reportLkpd.id, Number(params.id)))
      .limit(1);
...
export async function PUT(
  ...
  try {
    ...
    const updatedNews = await db.update(reportLkpd)
      .set(updateData)
      .where(eq(reportLkpd.id, id))
      .returning();
...
export async function DELETE(
  ...
  try {
    ...
    const [newsItem] = await db.select()
      .from(reportLkpd)
      .where(eq(reportLkpd.id, newsId)) // Now comparing number with number
      .limit(1);
    ...
    await db.delete(reportLkpd)
      .where(eq(reportLkpd.id, newsId));
