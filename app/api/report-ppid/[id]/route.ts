...
export async function PUT(
  ...
  try {
    ...
    const updatedNews = await db.update(reportPpid)
      .set(updateData)
      .where(eq(reportPpid.id, id))
      .returning();
...
export async function DELETE(
  ...
  try {
    ...
    const [newsItem] = await db.select()
      .from(reportPpid)
      .where(eq(reportPpid.id, newsId)) // Now comparing number with number
      .limit(1);
    ...
    await db.delete(reportPpid)
      .where(eq(reportPpid.id, newsId));
