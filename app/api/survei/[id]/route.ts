...
export async function GET(
  ...
  try {
    const singleNews = await db
      .select()
      .from(survei)
      .where(eq(survei.id, Number(params.id)))
      .limit(1);
...
export async function PUT(
  ...
  try {
    ...
    const updatedNews = await db.update(survei)
      .set(updateData)
      .where(eq(survei.id, id))
      .returning();
...
export async function DELETE(
  ...
  try {
    ...
    const [newsItem] = await db.select()
      .from(survei)
      .where(eq(survei.id, newsId)) // Now comparing number with number
      .limit(1);
    ...
    await db.delete(survei)
      .where(eq(survei.id, newsId));
