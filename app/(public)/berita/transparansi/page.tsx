...
export default function NewsPage() {
  ...
  useEffect(() => {
    setSearchInput(currentSearch); // Sync input dengan URL
  }, [currentSearch]);
...
function NewsCard({ item }: { item: News }) {
  return (
    <Link href={`/berita/transparansi/${item.id}`} passHref legacyBehavior>
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-all cursor-pointer">
        <div className="relative h-48 w-full">
          <Image
            src={`${process.env.NEXT_PUBLIC_SUPABASE_OBJECT_STORAGE}${item.imageUrl}`}
            alt={item.title}
            fill
