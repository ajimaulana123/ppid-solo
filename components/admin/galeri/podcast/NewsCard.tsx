...
import { DialogEditNews } from "@/components/admin/galeri/podcast/NewsFormEditDialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function NewsCard({ item, onNewsUpdated }: { 
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/podcast-ppid/${item.id}`, {
        method: 'DELETE'
      });

      if (!res.ok) throw new Error('Gagal menghapus Podcast');

      toast({
        title: "Berhasil",
        description: "Podcast telah dihapus",
        variant: "default"
      });
  ...
  return (
    <div className="group relative">
      ...
      {/* Konten laporan */}
      <Link href={`/admin/galeri/podcast/${item.id}`} passHref legacyBehavior>
