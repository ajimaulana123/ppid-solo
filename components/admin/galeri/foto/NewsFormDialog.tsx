...
export function DialogNews({ onNewsAdded }: { onNewsAdded?: () => void }) {
  ...
  const onSubmit = async (data: NewsSchema) => {
    ...
    try {
      ...
      const res = await fetch('/api/activities-image-ppid', {
        method: 'POST',
        body: formData
      });
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || 'Gagal menambahkan foto');
      }

      toast({
        title: "Berhasil",
        description: "Foto baru telah ditambahkan",
        variant: "default"
      });
      ...
      const errorMessage = error instanceof Error 
        ? error.message 
        : 'Terjadi kesalahan saat menambahkan foto';
