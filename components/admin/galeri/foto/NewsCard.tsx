...
import { DialogEditNews } from "@/components/admin/galeri/foto/NewsFormEditDialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function NewsCard({ item, onNewsUpdated }: { 
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/activities-image-ppid/${item.id}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="group relative">
      ...
      {/* Konten laporan */}
      <Link href={`/admin/galeri/foto/${item.id}`} passHref legacyBehavior>
