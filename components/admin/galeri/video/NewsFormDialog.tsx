...
export function DialogNews({ onNewsAdded }: { onNewsAdded?: () => void }) {
  ...
  const onSubmit = async (data: NewsSchema) => {
    ...
    try {
      ...
      const res = await fetch('/api/activities-video-ppid', {
        method: 'POST',
        body: formData
      });
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || 'Gagal menambahkan Video');
      }

      toast({
        title: "Berhasil",
        description: "Video baru telah ditambahkan",
        variant: "default"
      });
      ...
      toast({
        title: "Gagal",
        description: error instanceof Error ? error.message : 'Terjadi kesalahan saat menambahkan video',
        variant: "destructive"
      });
    } finally {
