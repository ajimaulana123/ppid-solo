...
import { DialogEditNews } from "@/components/admin/galeri/komik/NewsFormEditDialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function NewsCard({ item, onNewsUpdated }: { 
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/comic-ppid/${item.id}`, {
        method: 'DELETE'
      });
      ...
      toast({
        title: "Berhasil",
        description: "Komik telah dihapus",
        variant: "default"
      });
  ...
  return (
    <div className="group relative">
      ...
      {/* Konten laporan */}
      <Link href={`/admin/galeri/komik/${item.id}`} passHref legacyBehavior>
