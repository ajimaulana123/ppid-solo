...
export function DialogNews({ onNewsAdded }: { onNewsAdded?: () => void }) {
  ...
  const onSubmit = async (data: NewsSchema) => {
    ...
    try {
      ...
      const res = await fetch('/api/comic-ppid', {
        method: 'POST',
        body: formData
      });

      if (!res.ok) throw new Error('Gagal menambahkan Komik');

      toast({
        title: "Berhasil",
        description: "Komik baru telah ditambahkan",
        variant: "default"
      });
      ...
      toast({
        title: "Gagal",
        description: error instanceof Error ? error.message : 'Terjadi kesalahan saat menambahkan komik',
        variant: "destructive"
      });
    } finally {
