...
import { DialogEditNews } from "@/components/admin/laporan/ppid/NewsFormEditDialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function NewsCard({ item, onNewsUpdated }: { 
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/report-ppid/${item.id}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="group relative">
      ...
      {/* Konten laporan */}
      <Link href={`/admin/laporan/ppid/${item.id}`} passHref legacyBehavior>
