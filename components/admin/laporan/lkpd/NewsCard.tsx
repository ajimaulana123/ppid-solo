...
import { DialogEditNews } from "@/components/admin/laporan/lkpd/NewsFormEditDialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function NewsCard({ item, onNewsUpdated }: { 
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/report-lkpd/${item.id}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="group relative">
      ...
      {/* Konten laporan */}
      <Link href={`/admin/laporan/lkpd/${item.id}`} passHref legacyBehavior>
